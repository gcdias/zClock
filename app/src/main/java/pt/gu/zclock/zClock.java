...
public class zClock {
    ...
    public zClock(int clockMode,long NewDayTime, PointF dpClock) {
        ...
        this.raio = Math.min(pxClock.x/2-12, pxClock.y/2-12);
        if (clockMode<3) this.pxClock.x=raio*2; this.pxClock.y=raio*2;
        this.raio -= pad;
    ...
    public void updateTimeMarks() {
        ...
        raio = (int)(Math.min(pxClock.x / 2, pxClock.y / 2) - textMarksMaxWidth[0] - szFrame-4);
        this.patFrame = renderDashPathEffect(this.raio);
        timeMarkPaths();
    }
    ...
    private aPath getaPath(float r1, float r2, float angle, PointF center,boolean inside) {
        aPath apath = new aPath();
        Path p = new Path();
        angle += angle_offset;
        double cos = Math.cos(angle * Math.PI / 180F);
        double sin = Math.sin(angle * Math.PI / 180F);
        float x1 = (float) (r1 * cos) + centro.x;
        float y1 = (float) (r1 * sin) + centro.y;
        float x2 = (float) (r2 * cos) + centro.x;
        float y2 = (float) (r2 * sin) + centro.y;
        if (cos < 0) {
            p.moveTo(x2, y2);
            p.lineTo(x1, y1);
            apath.align = Paint.Align.RIGHT;
        } else {
            p.moveTo(x1, y1);
            p.lineTo(x2, y2);
            apath.align = Paint.Align.LEFT;
        }
        apath.path = p;
        return apath;
    }
    ...
    public boolean isNewDay(){
        return ((getTimeMins()/4)%360>=((newDayTimeMilis / 60000f) % 1440)/4);
    }
    public PointF getClockCenter(){
        return centro;
    }
